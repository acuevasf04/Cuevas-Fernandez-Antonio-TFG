...
* **LAN 2 (Linux - Aplicación Web / DMZ):**
    ...
    * Compuesta por un balanceador de carga Nginx, dos servidores backend PHP-FPM y un servidor de base de datos MySQL 8.0 aislado[cite: 494]. Tiene acceso entrante controlado desde el exterior por los puertos 80 y 443.
...
* **Cifrado HTTPS:** Redirección forzosa de HTTP a HTTPS (puerto 443) empleando TLS 1.2 o superior. Utiliza certificados SSL guardados en `./certs`[cite: 557]. El script `certificados.sh` monitoriza la caducidad y envía alertas por email si quedan menos de 30 días.
...
* **Punto Único de Fallo (SPOF):** La base de datos, al ser de nodo único, es el elemento crítico[cite: 383]. Se mitiga con políticas de reinicio automático (`restart: always`) en Docker y respaldos externos robustos.
